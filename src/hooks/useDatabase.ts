...
// Void Sale mutation - returns stock and marks sale as voided
export const useVoidSale = () => {
  ...
  return useMutation({
    mutationFn: async ({ saleId, reason }: { saleId: number; reason: string }) => {
      // 1. Get sale items to know what stock to return
      const { data: saleItems, error: itemsError } = await supabase
        .from('sale_items')
      ...
      // 2. Create return_in stock movements for each item
      const stockMovements = saleItems.map(item => ({
        product_id: item.product_id,
        movement_type: 'return_in' as const,
        quantity: item.quantity, // Positive quantity to add back to stock
        unit_cost: item.unit_cost,
    ...
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['sales'], refetchType: 'all' }),
        queryClient.invalidateQueries({ queryKey: ['transactions'], refetchType: 'all' }),
        queryClient.invalidateQueries({ queryKey: ['transaction-details'], refetchType: 'all' }),
        queryClient.invalidateQueries({ queryKey: ['sold-items-report'], refetchType: 'all' }),
        queryClient.invalidateQueries({ queryKey: ['products'], refetchType: 'all' }),
        queryClient.invalidateQueries({ queryKey: ['stock-data'], refetchType: 'all' }),
        queryClient.invalidateQueries({ queryKey: ['inventory-values'], refetchType: 'all' }),
      ...
      toast({
        title: "Sale Voided",
        description: "The sale has been voided and stock has been returned to inventory"
      });
    },
    onError: (error) => {
      toast({
...
// Update Sale Item mutation - allows editing price, quantity, discount
export const useUpdateSaleItem = () => {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      saleId: number;
      productId: number;
      updates: { unit_price?: number; quantity?: number; discount?: number };
      originalQuantity: number;
    }) => {
      // 1. Update the sale item
      ...
      // 2. If quantity changed, adjust stock
      if (updates.quantity !== undefined && updates.quantity !== originalQuantity) {
        const quantityDiff = originalQuantity - updates.quantity;

        if (quantityDiff > 0) {
          // Quantity decreased - return stock (return_in)
          const { error: movementError } = await supabase
            .from('stock_movements')
            .insert({
          ...
          // Quantity increased - remove more stock (sale)
          const { error: movementError } = await supabase
            .from('stock_movements')
            .insert({
              product_id: productId,
      ...
      // Note: Sale totals are auto-recalculated by database trigger
      return { success: true };
    },
    onSuccess: async () => {
      await Promise.all([
