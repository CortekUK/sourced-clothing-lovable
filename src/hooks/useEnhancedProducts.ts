  ...
  return useQuery({
    ...
    queryFn: async (): Promise<ProductWithStock[]> => {
      ...
      if (filters) {
        ...
        // Fabric filter (stored in metal column)
        if (filters.fabrics.length > 0) {
          query = query.in('metal', filters.fabrics);
        }

        // Size filter (stored in karat column)
        if (filters.sizes.length > 0) {
          query = query.in('karat', filters.sizes);
        }

        // Color filter (stored in gemstone column)
        if (filters.colors.length > 0) {
          query = query.in('gemstone', filters.colors);
        }
