...
export function ProductSearch({ onAddToCart, cartItems }: ProductSearchProps) {
  ...
  // Determine which products to display - only show IN STOCK items
  const products = useMemo(() => {
    if (searchQuery) {
      // Search results already have stock_on_hand merged in from useProductSearch
      // Filter to only show in-stock products
      return searchResults.filter(product => {
        const stockOnHand = product.stock_on_hand || 0;
        // Show if not tracking stock OR has stock available
        return !product.track_stock || stockOnHand > 0;
      });
    }
