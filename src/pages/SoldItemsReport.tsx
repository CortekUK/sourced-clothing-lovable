...
export default function SoldItemsReport() {
  ...
  const filterOptions = useMemo(() => {
    ...
    soldItemsData.forEach((item: any) => {
      ...
      if (item?.products?.metal) fabrics.add(item.products.metal);
    ...
    return {
      ...
      fabrics: Array.from(fabrics) as string[],
  ...
  const filteredItems = useMemo(() => {
    ...
    return soldItemsData.filter((item: any) => {
      ...
      // Category and fabric filters
      const matchesCategory = !filters.category || filters.category === 'all' || filters.category === '' || item?.products?.category === filters.category;
      const matchesFabric = !filters.fabric || filters.fabric === 'all' || filters.fabric === '' || item?.products?.metal === filters.fabric;
      ...
      return matchesDateRange && matchesProductSearch && matchesCategory && matchesFabric && matchesStaff && matchesSupplier && matchesSale && matchesFlags;
    });
  ...
  const totals = useMemo(() => {
    if (!filteredItems || !Array.isArray(filteredItems)) {
      return { revenue: 0, cogs: 0, profit: 0, items: 0, tradeInCount: 0, voidedCount: 0 };
    }
    ...
    return filteredItems.reduce(
      ...
      { revenue: 0, cogs: 0, profit: 0, items: 0, tradeInCount, voidedCount }
    );
              ...
              <CardTitle className="text-sm font-medium flex items-center gap-2 flex-wrap">
                ...
                {totals.voidedCount > 0 && (
                  <Badge variant="outline" className="text-xs font-normal border-destructive/50 text-destructive">
                    ...
                    {totals.voidedCount} voided excluded
                  </Badge>
                )}
              </CardTitle>
        ...
        <Card className="shadow-card border-primary/10">
          ...
          <CardContent className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="fabric-filter">Fabric Type</Label>
                <Select
                  value={filters.fabric}
                  onValueChange={(value) => setFilters(prev => ({ ...prev, fabric: value }))}
                >
                  <SelectTrigger>
